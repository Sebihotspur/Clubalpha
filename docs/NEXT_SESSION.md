...
- The latest cumulative research checkpoint is frozen at
  `artifacts/research_loop/2026-09-05-17-completed/`. Fourteen teams now have
  two observations; the six teams in the three unfinished fixtures have one.
...
- The Ledger now includes a collapsed Matchweek History view. Each week keeps
  ...
  the official promotion gate. Seven Matchweek 3 fixtures are now settled:
  official 1X2 is 2/7, raw probability-leader 1X2 is 1/7, O/U 2.5 is 2/7,
  and BTTS is 4/7. Three fixtures remain pending.
...
- Across the seven official results, mean total xG was close at 3.02 forecast
  versus 3.07 observed. The distribution was badly compressed: forecasts
  ranged only 2.76–3.19 while observed xG ranged 1.39–5.10. Five fixtures are
  structural reviews; Fulham–Palace is an outcome miss whose 3.13–1.97 xG
  process still supported Fulham.
- The raw probability model selected the home side in all seven settled
  fixtures; only Manchester City won. This is evidence to audit the
  home/favorite translation, not permission to add an after-the-fact draw
  adjustment.
- Projected lineups averaged 8.29/11 correct. Tottenham fell to 5/11 against
...
- The public Methodology page now exposes the learning loop, its 17/20 frozen
...
- All 180 tests pass, including regression guards proving the earlier Holy
...
1. As each remaining Matchweek 3 fixture settles, append results to the official archive
   ...
   ```
2. Audit why the raw probability model chose the home side in all seven
   settled official fixtures. Separate the 1X2 translation problem from the
   mean goal-environment estimate, which was close on average.
