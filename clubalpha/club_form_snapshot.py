...
def join_club_form_snapshots(
    forms: Iterable[dict[str, Any]], dynamics: Iterable[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    if set(forms_by_team) != set(dynamics_by_team):
        missing = sorted(set(forms_by_team) - set(dynamics_by_team))
        extra = sorted(set(dynamics_by_team) - set(forms_by_team))
        raise ValueError(
            f"Club Form component universes differ: missing dynamics={missing}; extra dynamics={extra}"
        )

    snapshots: list[dict[str, Any]] = []
    for team_id, form in forms_by_team.items():
        dynamic = dynamics_by_team[team_id]
        if form.get("as_of") != dynamic.get("as_of"):
            raise ValueError(
                f"Snapshot date mismatch for team_id={team_id}: "
                f"form={form.get('as_of')} dynamics={dynamic.get('as_of')}"
            )
        if form.get("team") != dynamic.get("team"):
            raise ValueError(
        ...
        blockers = [
            "expected_starting_xi_not_built",
            "expected_minutes_not_built",
