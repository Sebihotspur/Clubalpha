...
def selection(team_id=10, team="Club", as_of="2026-08-18"):
    return {
        "selection_prior_version": "selection-v1",
        "as_of": as_of,
        "team_id": team_id,
        "team": team,
        "shape_prior": {"formation": "4-3-3"},
        "expected_starting_xi_prior": [{"player_id": index} for index in range(11)],
        "players": [],
        "evidence": {"recent_matches": 3},
        "decision_boundaries": {"lineup_prior_ready": True},
        "quality_flags": [],
    }
...
class ClubFormSnapshotTests(unittest.TestCase):
    def test_join_preserves_components_without_creating_a_score(self):
        row = join_club_form_snapshots([form()], [dynamics()], [selection()])[0]
        ...
        self.assertTrue(row["decision_boundaries"]["selection_prior_built"])
        self.assertTrue(row["decision_boundaries"]["expected_minutes_prior_built"])
        self.assertFalse(row["decision_boundaries"]["expected_minutes_locked"])
    ...
    def test_component_universes_must_match(self):
        with self.assertRaisesRegex(ValueError, "component universes differ"):
            join_club_form_snapshots([form()], [], [selection()])

    def test_selection_universe_must_match(self):
        with self.assertRaisesRegex(ValueError, "component universes differ"):
            join_club_form_snapshots([form()], [dynamics()], [])

    def test_missing_selection_evidence_remains_a_projection_blocker(self):
        prior = selection()
        prior["decision_boundaries"]["lineup_prior_ready"] = False
        row = join_club_form_snapshots([form()], [dynamics()], [prior])[0]
        self.assertFalse(row["decision_boundaries"]["expected_minutes_prior_built"])
        self.assertIn(
            "squad_selection_evidence_missing",
            row["decision_boundaries"]["projection_blockers"],
        )
